...
angular.module('core').controller('componentCtrl', ['$scope', '$http', '$rootScope', '$uibModal', 'commonService', '$timeout', '$compile', '$state', "$stateParams",
    function ($scope, $http, $rootScope, $uibModal, commonService, $timeout, $compile, $state, $stateParams) {
        ...
        * */
        ...
        window.componentListUpdata = function componentListUpdata() {
            ...
            $scope.data.componentList.map(function (val) {
                ...
                val.proContiner = false;
                val.staBtnVerson = false;
                val.statusVerson = false;
                val.staBtnApply = false;
                if (status == 0) {//下载
                    val.staBtnLoad = true;
                    //右上角已下载
        ...
        function getDom() {
            ...
            $scope.data.componentList.map((itemList) => {
                let itemLists = JSON.stringify(itemList);
                //componentListUpdata();
                let html = `<li sta="${itemList.subClassName},${itemList.guid},${itemList.md5},${itemList.epId},${itemList.originEpId},${itemList.lastestClientVersion}">
                        <div class="preview-model" name="${itemList.componentId},${itemList.epId},${itemList.originEpId}">
                            <div class="statusLable">已下载</div>
                            <div class="statusVerson">版本过高</div>
                            <div class="proContiner" style="position: absolute;left: 0;top: 0;width: 240px;height: 277px;background: rgba(0,0,0,0.5)">
                            ...
                            <div class="preview-big"><img src="imgs/placeholder.png" _src="${itemList.attachmentInfo[0].displayUrl}?${name = itemList.attachmentInfo[0].fileName}"/></div>
                        </div>
                        <div class="preview-tool">
                            ...
                            <a style="cursor: pointer"><span class="icon-describe"></span>
                                <i class="staBtnLoad staBtn">下载</i>
                                <i class="staBtnData staBtn">更新</i>
                                <i class="staBtnApply staBtn" style="cursor: pointer">应用</i>
                                <i class="staBtnVerson staBtn">版本过高</i>
                            </a>
                        </div>
                    </li>`
                ...
                $('.component-info .clearfix>li:last-child').attr("obj", itemLists);
                getBtnStatus();
            })
            $(".component-info .clearfix .preview-big").map((i, v) => {
                $(v).click((e) => {
                    let obj = $(v).parents("li").attr("obj");
                    obj = JSON.parse(obj);
                    openModel(obj, $(v));
                })
            })
            ...
            //下载
            $(".component-info .clearfix .preview-tool .staBtnLoad").map((i, v) => {
                $(v).click((e) => {
                    let obj = $(v).parents("li").attr("obj");
                    obj = JSON.parse(obj);
                    obj.proContiner = true;
                    $(v).parents("li").find(".proContiner").show();
                    BzCloudComp.DownloadComp(obj.componentId, obj.epId, obj.originEpId, obj.epName, obj.originEpName);
                })
            })
            //更新
            $(".component-info .clearfix .preview-tool .staBtnData").map((i, v) => {
                $(v).click((e) => {
                    let obj = $(v).parents("li").attr("obj");
                    obj = JSON.parse(obj);
            ...
            //应用
            $(".component-info .clearfix .preview-tool .staBtnApply").map((i, v) => {
                $(v).click((e) => {
                    let obj = $(v).parents("li").attr("obj");
                    obj = JSON.parse(obj);
                    BzCloudComp.UseComp(obj.subClassName, obj.guid, obj.epId, obj.originEpId);
                })
            })
        }

        function getBtnStatus() {
            $('.component-info .clearfix>li').map((i, v) => {
                let _this = $(v);
                let val = $(v).attr("obj");
                val = JSON.parse(val);
                var status = BzCloudComp.GetCompType(val.subClassName, val.guid, val.md5, val.epId - 0, val.originEpId - 0, val.lastestClientVersion, val.componentId);
                _this.find(".staBtnLoad").hide();
                _this.find(".statusLable").hide();
                _this.find(".staBtnData").hide();
                _this.find(".staBtnApply").hide();
                _this.find(".proContiner").hide();
                _this.find(".staBtnVerson").hide();
                _this.find(".statusVerson").hide();

                if (status == 0) {//下载
                    _this.find(".staBtnLoad").show();
                    //右上角已下载
                    val.statusLable = false;
                    //更新按钮
                    val.staBtnData = false;
                } else if (status == 1) {//更新
                    _this.find(".staBtnData").show();
                    val.staBtnApply = false;
                    val.statusLable = false;
                    val.staBtnLoad = false;
                } else if (status == 2) {//应用
                    _this.find(".staBtnApply").show();
                    _this.find(".statusLable").show();
                    val.staBtnApply = true;
                    val.statusLable = true;
                } else if (status == 3) {
                    _this.find(".staBtnVerson").show();
                    _this.find(".statusVerson").show();
                    val.staBtnVerson = true;
                    val.statusVerson = true;
                } else if (status < 0) {
                    _this.find(".proContiner").show();
                    val.proContiner = true;
                }
            })
        ...
        //应用
        $scope.apply = (subClassName, guid, epId, originEpId) => {
            BzCloudComp.UseComp(subClassName, guid, epId, originEpId);
        }
        //下载
        $scope.down = (componentId, epId, originEpId, epName, originEpName, subClassName, guid, md5, lastestClientVersion, obj) => {
            obj.proContiner = true;
        ...
        function openModel(obj, e) {
            ...
            modalInstance.result.then(function (selectedItem) {
                if (selectedItem != "" && selectedItem != undefined) {
                    obj.proContiner = true;
                    e.parents("li").find(".proContiner").show();
                }
        ...
        * 初始化参数配置
        * */
        $scope.componetModal = (componentId, epId, obj) => {
            console.log(componentId, epId, obj);
            var scope;
            if (epId == -2) {
                scope = 1
            } else {
